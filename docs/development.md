...
```
cmd/http-echo-server/   entrypoint
...
make build         # build ./bin/http-echo-server (version injected via ldflags)
...
sequenceDiagram
    ...
    GR->>GH: push multi-arch images to ghcr.io/kumy/http-echo-server
```

- Version bumps come from commit types (`fix:` → patch, `feat:` → minor,
  `!`/`BREAKING CHANGE` → major) — see
  [CONTRIBUTING](https://github.com/kumy/http-echo-server/blob/main/CONTRIBUTING.md).
